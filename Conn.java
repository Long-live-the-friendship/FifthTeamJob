...
public class Conn {
    ...
    public int delete(Book book) throws SQLException {
        try {
            this.connect();
            String sql = "delete from booktb where id = ? and name = ?";
            PreparedStatement pstmt;
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, String.valueOf(book.getId()));
            pstmt.setString(2, String.valueOf(book.getName()));
    ...
    public int modify(Book book) throws SQLException {
        try {
            this.connect();
            String sql = "update book set book.name = ? where book.id = ?";
            PreparedStatement pstmt;
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(2, String.valueOf(book.getId()));
            pstmt.setString(1, String.valueOf(book.getName()));
            int result = pstmt.executeUpdate();;
            pstmt.executeUpdate();
    ...
    public void rec(Book book)  {
        ...
        try {
            this.connect();
            String sql = "SELECT name FROM book ORDER  BY  RAND() LIMIT 1";
            Statement stmt = conn.createStatement();
            ResultSet result = stmt.executeQuery(sql);
            while (result.next()) {
                String name = result.getString("name");
                System.out.println("好书推荐：" + name);
                }
            conn.close();
            result.close();
            stmt.close();
